...
protected:
	...
	virtual void insertar(ClaveValor const& cv, Link & a, bool& repetido);
	...
	static int k_elem(Link & a, int& elems);
...
void TreeMap<Clave, Valor, Comparador>::insert(Clave const& clave, Valor const& valor) {
	bool repetido = false;
	insertar({ clave, valor }, raiz, repetido);
}
...
template <typename Clave, typename Valor, typename Comparador>
int TreeMap<Clave, Valor, Comparador>::k_elem(int elem){
	int valor;
	valor = k_elem(this->raiz, elem);
...
public:
	...
	TreeNode(ClaveValor e, Link i = nullptr, Link d = nullptr, int alt = 1)
		: cv(e), iz(i), dr(d), altura(alt), tam_i(1) {}
};
...
template <typename Clave, typename Valor, typename Comparador>
void TreeMap<Clave, Valor, Comparador>::insertar(ClaveValor const& cv, Link & a, bool & repetido) {
	if (a == nullptr) {
		a = new TreeNode(cv);
	}
	else if (menor(cv.clave, a->cv.clave)) {
		insertar(cv, a->iz, repetido);
		if (!repetido) a->tam_i = a->tam_i + 1;
		reequilibraDer(a);
	}
	else if (menor(a->cv.clave, cv.clave)) {
		insertar(cv, a->dr, repetido);
		reequilibraIzq(a);
	}
...
template <typename Clave, typename Valor, typename Comparador>
int TreeMap<Clave, Valor, Comparador>::k_elem(Link & a, int& elems){
	if (a->tam_i == elems) return a->cv.clave;
	if (a->tam_i > elems) return k_elem(a->iz, elems);

	if (a->tam_i < elems){
		elems = elems - a->tam_i;
		if (a->dr == nullptr) return -1;
		else return k_elem(a->dr, elems);
	}
}
template <typename Clave, typename Valor, typename Comparador>
